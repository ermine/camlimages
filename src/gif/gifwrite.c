...
CAMLprim value eGifOpenFileName(value name) {
  CAMLparam1(name);
...
CAMLprim value eGifCloseFile(value hdl) {
  CAMLparam1(hdl);
...
CAMLprim value eGifPutScreenDesc(value oc, value sdesc) {
  CAMLparam2(oc,sdesc);
...
CAMLprim value eGifPutImageDesc(value oc, value idesc) {
  CAMLparam2(oc,idesc);
  ...
  if (EGifPutImageDesc(GifFileOut,
                       ...
                       (const ColorMapObject *) ColorMapObject_val(Field(idesc, 5))) == GIF_ERROR) {
    failwith("EGifPutImageDesc");
  }
  CAMLreturn(Val_unit);
}

CAMLprim value eGifPutLine(value oc, value buf) {
  CAMLparam2(oc,buf);

  GifFileType *GifFileOut = (GifFileType*) oc;

  if (EGifPutLine(GifFileOut, (GifPixelType *) String_val(buf), 
...
CAMLprim value eGifPutExtension(value oc, value ext) {
  CAMLparam2(oc,ext);
  CAMLlocal1(l);
